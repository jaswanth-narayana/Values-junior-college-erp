...
CREATE TABLE IF NOT EXISTS students(
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  student_code VARCHAR(40) UNIQUE DEFAULT('VJC-'||TO_CHAR(NOW(),'YYYY')||'-'||UPPER(SUBSTR(REPLACE(gen_random_uuid()::text,'-',''),1,6))),
...
CREATE TABLE IF NOT EXISTS student_fee_allocations(
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS payments(
  ...
  allocation_id UUID REFERENCES student_fee_allocations(id),
  receipt_number VARCHAR(50) UNIQUE DEFAULT('VJC-R-'||TO_CHAR(NOW(),'YYYYMMDD')||'-'||NEXTVAL('payments_receipt_seq')),
  payment_date DATE DEFAULT CURRENT_DATE,
  amount_paid NUMERIC(12,2) CHECK(amount_paid>0),
  payment_mode VARCHAR(30) CHECK(payment_mode IN('Cash','UPI','Card','Bank Transfer')),
...
CREATE TABLE IF NOT EXISTS student_transport(
  ...
  transport_fee_id UUID REFERENCES fees(id),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(student_id)
);
...
CREATE TABLE IF NOT EXISTS college_settings(
  ...
  college_name VARCHAR(160) NOT NULL DEFAULT 'Values Junior College',
  address TEXT,
  phone VARCHAR(20),
  email VARCHAR(160),
  logo_url TEXT,
...
INSERT INTO users(username,name,email,password_hash,role) VALUES('admin','Admin Staff','admin@values.edu',crypt('Admin@123',gen_salt('bf')),'super_admin') ON CONFLICT DO NOTHING;
INSERT INTO college_settings(id,college_name,address,phone,email) VALUES(1,'Values Junior College','Hyderabad, Telangana, India','+91 40 2345 6789','office@values.edu') ON CONFLICT(id) DO NOTHING;